...
import it.unipr.ce.dsg.patrol.gui.message.content.GamePeerInfo;
...
import it.unipr.ce.dsg.patrol.util.CheckOutput;
...
response=it.simplexml.sender.MessageSender.sendMessage("127.0.0.1",this.portnumber+5, message.generateXmlMessageString());
    ...
    MessageReader messageReader=new MessageReader();
    Message receivedMessage = messageReader.readMessageFromString(response.trim());
    //MultiLog.println(RTSGameGUI.class.toString(),"Received message: /"+receivedMessage.getMessageType()+"/ /"+receivedMessage.getSourceName()+"/ /"+receivedMessage.getSourceSocketAddr()+"/ /"+receivedMessage.getSourcePort()+"/ /"+receivedMessage.getPARAMETERS_NUM());
    /*for(int cont=0;cont<receivedMessage.getPARAMETERS_NUM();cont++)
        MultiLog.println(RTSGameGUI.class.toString(),"\t"+receivedMessage.getParametersList().get(cont).getName()+"/ /"+receivedMessage.getParametersList().get(cont).getValue());*/
    if(receivedMessage.getMessageType().equals("GPVISION"))
    {
        ...
        //MultiLog.println(RTSGameGUI.class.toString(),"gpv.getVision: "+str_vision);
        ArrayList<Object> vision=new ArrayList<Object>();

        String []Array_v=str_vision.split("\\$");
        /*MultiLog.println(RTSGameGUI.class.toString(),"Array_v (split by \\$) ["+Array_v.length+"]: ");
        for(int cont=0;cont<Array_v.length;cont++)
            MultiLog.println(RTSGameGUI.class.toString(),"\t"+Array_v[cont]);*/
        int nv=Array_v.length;
        /*
         * Rebuilds th(int)Math.round(y_pos*this.numTileX + x_pos)e vision information list of the game peer 
         */
        for(int j=0;j<nv;j++)
        {
            String [] Array_vision=Array_v[j].split("\\;");
            /*MultiLog.println(RTSGameGUI.class.toString(),"Array_vision (split by \\;) ["+Array_vision.length+"]: ");
            for(int cont=0;cont<Array_vision.length;cont++)
                MultiLog.println(RTSGameGUI.class.toString(),"\t"+Array_vision[cont]);*/
            String type=Array_vision[0];
                        if(type.equals("GamePlayerResponsible"))
                        {
                            int pos=Integer.parseInt(Array_vision[1]);
