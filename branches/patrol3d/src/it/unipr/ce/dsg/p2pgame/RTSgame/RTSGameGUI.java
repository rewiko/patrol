    	...
    	if (defenseQt.isEditable()){
         ...
         //   resList.setList(gp.getMyResources());
           // peerGUI.getResourcesList().setModel(resList);
    		...
    		//GameResourceMobile att = new GameResourceMobile("att" + timestamp, "Attack" + timestamp, Double.parseDouble(attackQt.getText()));
    		...
    		//gp.publishResource("ste");

    		//gp.addToMyResource(att);
    		peerGUI.getResourcesList().setModel( new GameResourceListModel(gp.getMyResources()));
    		peerGUI.repaint();
...
//									long time, String pos, String oldPos
    ...
    private static void setPlanetVision(ArrayList<Object> vision, GamePlayer player, double gran){
    	...
    	GamePlayerResponsible planet = new GamePlayerResponsible(gp.getPlayer().getId(), gp.getPlayer().getName(), gp.getPlayer().getPosX(),gp.getPlayer().getPosY(),0, 0,10,0,"","" );
		if (peerGUI.getSpacePanel() != null)
			peerGUI.getSpacePanel().addMyPlanetPosition( (int) Math.round(gp.getPlayer().getPosX()), (int) Math.round(gp.getPlayer().getPosY()),planet);
